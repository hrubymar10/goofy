...
function updateTitle() {
    var e = document.querySelector(LAST_ACTIVE) ? document.querySelector(LAST_ACTIVE).textContent : "";
    window.webkit.messageHandlers.notification.postMessage({
        type: "SET_TITLE",
        title: document.querySelector(CURRENT_CONVERSATION_NAME).textContent,
        activity: e
    })
}
    ...
    if (c > 0) {
        ...
        if (text) {
            ...
            if (lastNotification != subtitle + text) {
                ...
                if (emojiMessage) {
                    emojiMessage.textContent = findSurrogatePair(parseInt(document.querySelector(UNREAD_MESSAGE_TEXT + ' ' + EMOTICONS).getAttribute('src').split('/').reverse()[0].split('.')[0], 16)).map(function(a) {
